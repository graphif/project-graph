...
在 `Settings.tsx` 中扩展 Zod schema，添加快捷编辑相关设置项，使用数组存储快捷编辑操作：
...
// 快捷编辑设置
quickEdit: {
  operations: QuickEditOperation[]; // 数组形式，共7个元素
}
```
...
```typescript
// 通用快捷编辑方法
static quickEdit(project: Project, operationIndex: number) {
  // 获取配置
  const config = project.settings.quickEdit.operations[operationIndex];
...
在 `shortcutKeysRegister.tsx` 中直接写出七个快捷操作的注册对象到现有的快捷键数组中：
...
// 快捷编辑操作快捷键 - 直接写入七个对象
{
  ...
  sequence: [],
  timeout: 1000,
  callback: () => TextNodeSmartTools.quickEdit(project!, 0),
  description: "快捷编辑操作 1"
},
{
  id: "quickEdit2",
  sequence: [],
  timeout: 1000,
  callback: () => TextNodeSmartTools.quickEdit(project!, 1),
  description: "快捷编辑操作 2"
},
// ... 共7个
```
...
1. **扩展设置项定义**：在 `Settings.tsx` 中添加快捷编辑相关的Zod schema，使用数组存储操作配置
2. **添加设置项UI**：在 `settings.tsx` 中添加"快捷编辑文本节点"子栏目和7个设置项
3. **重构文本节点工具**：修改 `textNodeSmartTools.tsx`，实现通用的快捷编辑方法
4. **注册快捷键**：在 `shortcutKeysRegister.tsx` 中直接添加7个快捷操作的注册对象
...
1. `/app/src/core/service/Settings.tsx` - 扩展设置项定义，使用数组存储快捷编辑操作
...
4. `/app/src/core/stage/shortcutKeysRegister.tsx` - 直接添加7个快捷操作的注册对象
