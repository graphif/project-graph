...
func _process(_delta: float) -> void:
	...
	var source_inner := source.position + source.size * source_uv
	var target_inner := target.position + target.size * target_uv
	...
	var control_distance: float = maxf(
		line.width * 25.0, minf(absf(offset.x), absf(offset.y)) / 2.0
	)
...
func _get_connection_point(entity: Entity, uv: Vector2, direction: Vector2) -> Vector2:
	var inner := entity.position + entity.size * uv
...
func _get_rectangle_intersection(entity: Entity, direction: Vector2) -> Vector2:
	var center := entity.position + entity.size / 2.0
	if direction.length_squared() <= 0.001:
		return center

	var half_size := entity.size / 2.0
...
func _get_rectangle_normal(
	entity: Entity, point: Vector2, fallback: Vector2
) -> Vector2:
